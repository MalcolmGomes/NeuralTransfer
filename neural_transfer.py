...
def run_style_transfer(cnn, normalization_mean, normalization_std,
    ...
    while run[0] <= num_steps:
        ...
        def closure():
            ...
            run[0] += 1
            if run[0] % 100 == 0: print("run {}:".format(run), 'Style Loss : {:4f} Content Loss: {:4f}'.format(style_score.item(), content_score.item()), '\n')
...
if __name__ == '__main__':
    existing_images = input("Use existing images? (y/n): ")
    if(existing_images.lower() == 'y'):
        preset = input("Use preset images? (y/n):")
        if(preset.lower() == 'y'):
    ...
    output_image.save('outputs/' + filename)       
    input("Result saved in outputs folder. Press any key to exit.")